(ns asme_materials.main
  "Kotodama WASM entrypoint for the asme_materials clean-room actor (L4) — Clojure port.
  ...
  facts (`asme_materials.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "asme_materials")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"asme_materials.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "asme_materials-compat" :tier tier :entities entities} 200])
...
(defn start! [] :asme_materials-compat/ready)
